...
let PersonComponent = class PersonComponent {
    constructor(router, rs) {
        ...
        this.requestService.get('persons').subscribe((result) => {
            console.log(result.json());
            this.pesrons = result.json();
        });
    }
    Delete(id) {
        this.requestService.delete('persons/' + id).subscribe((resp) => {
            this.pesrons = this.pesrons.filter(x => x.Id !== id);
        });
    }
};
